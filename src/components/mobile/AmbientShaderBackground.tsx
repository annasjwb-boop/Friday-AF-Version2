...
import "./AmbientShaderBackground.css";

/**
 * Theme palette (low -> high): the app's icy-blue-over-white scheme, widened
 * with extra blue tones so the mesh has more range and texture. Each colour
 * becomes its own drifting blob in shade(), so more stops = a richer mix.
 * The low colour is white so the mesh still reads predominantly white with
 * blue accents drifting through. Values are 0..1 sRGB triples fed to u_colors.
 * The shader is light-mode only; dark mode hides it for a solid black backdrop.
 */
const THEME_COLORS: [number, number, number][] = [
  [1.0, 1.0, 1.0], // #FFFFFF white base
  [0.894, 0.969, 1.0], // #E4F7FF pale cyan
  [0.706, 0.925, 0.984], // #B4ECFB icy cyan
  [0.663, 0.839, 1.0], // #A9D6FF light blue
  [0.471, 0.722, 1.0], // #78B8FF azure
  [0.431, 0.608, 1.0], // #6E9BFF periwinkle
  [0.243, 0.518, 0.965], // #3E84F6 deep blue accent
  [0.416, 0.294, 0.941], // #6A4BF0 indigo-violet accent
];
...
/**
 * Animated WebGL "Mesh drift" (blobs) background, rendered in a plain WebGL1
 * context with a fullscreen triangle and no libraries. Colours are matched to
 * the app theme (icy-blue ramp). Mounted behind all content, ignores pointer
 * events, caps devicePixelRatio at 2, pauses when the tab is hidden, and holds
...
export function AmbientShaderBackground({ veil = true }: { veil?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (!gl) return; // No WebGL: the white overlays remain as a graceful fallback.
    ...
    // u_colors[8] — the theme ramp fills the first THEME_COLORS.length slots,
    // the rest zeroed.
    ...
    gl.uniform4f(uShape, 1.1, 0.34, 0.5, 0.0); // scale, intensity, paramA, warp
    gl.uniform4f(uSurface, 2.4, 0.96, 0.0, 0.96); // detail, contrast, brightness, saturation
    gl.uniform4f(uFinish, 0.0, 0.0, 0.026, 0.07); // hue, vignette, blur, grain
    ...
    const draw = (seconds: number) => {
      ...
      gl.uniform4f(uScene, vw, vh, seconds * 0.73, colorCount);
  ...
  return (
    <div className="ambient-shader" aria-hidden="true">
      <canvas ref={canvasRef} className="ambient-shader__canvas" />

      {/* Full-frame mode only: white veils that keep content readable over
          the mesh. Band mode skips them so the color stays present. */}
      {veil && (
        <>
          {/* Downward white fade so the lower content resolves toward white
              and stays on-theme / readable. */}
          <div className="ambient-shader__wash" />

          {/* Subtle radial white mask that keeps contrast behind the primary
              cards. */}
          <div className="ambient-shader__cards-mask" />
        </>
      )}
    </div>
  );
