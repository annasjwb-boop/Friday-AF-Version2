...
export function ProfileDemoControl({
  ...
  return (
    <motion.div
      className="sanctuary-b-demo"
      initial={{ opacity: 0, height: 0 }}
      animate={{ opacity: 1, height: "auto" }}
      exit={{ opacity: 0, height: 0 }}
      transition={{ duration: 0.24, ease: [0.3, 0, 0.2, 1] }}
    >
      <span className="sanctuary-b-demo__label">Demo · profile scores</span>
      {DIMENSIONS.map(({ id, label }) => {
        ...
        return (
          <div
            key={id}
            className={`sanctuary-b-demo__slider-row${on ? "" : " is-muted"}`}
          >
            <span className="sanctuary-b-demo__slider-label">{label}</span>
            <input
              type="range"
              min={0}
              max={100}
            ...
            />
            <span className="sanctuary-b-demo__slider-value">
              {v === null ? "—" : v}
            </span>
            <button
              type="button"
              className={`sanctuary-b-demo__chip${v === null ? " is-active" : ""}`}
              aria-pressed={v === null}
            ...
            <button
              type="button"
              className={`sanctuary-b-demo__chip${on ? "" : " is-active"}`}
              aria-pressed={!on}
      ...
      <div className="sanctuary-b-demo__footer">
        <button
          type="button"
          className={`sanctuary-b-demo__chip${value.confirmedDamage ? " is-active" : ""}`}
        ...
        <Link to="/sanctuary-lab" className="sanctuary-b-demo__chip">
