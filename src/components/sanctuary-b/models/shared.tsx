...
function getWearTexture(bucket: number): CanvasTexture {
  ...
  for (let i = 0; i < blotches; i++) {
    ctx.fillStyle = `rgba(84, 72, 58, ${0.05 + Math.random() * 0.045 * bucket})`;
    ctx.beginPath();
    ctx.ellipse(
      Math.random() * size,
      Math.random() * size,
  ...
  for (let i = 0; i < streaks; i++) {
    ctx.fillStyle = `rgba(70, 62, 52, ${0.03 + 0.025 * bucket})`;
    ctx.fillRect(
      Math.random() * size,
  ...
  if (bucket >= 3) {
    for (let i = 0; i < 120; i++) {
      ctx.fillStyle = `rgba(60, 52, 44, ${0.08 + Math.random() * 0.1})`;
      ctx.fillRect(Math.random() * size, Math.random() * size, 2, 2);
    }
  }
