...
export function SkyModel({ params }: { params: StateParams }) {
  ...
  return (
    <group>
      {/* Main island: faceted rock, point down, city on the flat top */}
      <group rotation={[0, 0, damaged ? -0.05 : 0]}>
        <Shard p={[0, 0.95, 0]} r={[Math.PI, 0.3, 0]} radius={0.78} h={0.95} color={CLAY.cream} sides={7} />
        <Cyl p={[0, 1.46, 0]} rTop={0.72} rBot={0.76} h={0.08} color={CLAY.cream} />
        ...
        <DecoTower
          ...
          glow={glow * 0.9}
        />
        <Cyl p={[-0.45, 1.65, 0.28]} rTop={0.09} rBot={0.11} h={0.32} color={CLAY.cream} />
        <Spire p={[-0.45, 1.9, 0.28]} h={0.18} radius={0.025} />
        <Tree p={[0.12, 1.5, 0.42]} s={0.6} />
        <Tree p={[-0.5, 1.5, -0.35]} s={0.5} />

        {/* Cracks on the rock face; gold while healing */}
        {(wear >= 0.3 || recovering) && (
          <Seam
            p={[0.3, 1.1, 0.5]}
            r={[0.4, 0, 0.5]}
        ...
        {wear >= 0.6 && !recovering && (
          <Seam p={[-0.35, 0.95, 0.45]} r={[0.4, 0, -0.55]} length={0.24} />
        )}
      </group>
