...
/**
 * Owns the dissolve-and-reform choreography. When `targetId` changes, the
 * current sanctuary rotates away, sinks and collapses while the lighting
 * dims (via the shared `presenceRef`), dust lifts off the base, and the new
 * sanctuary rises into place. Also carries the idle breathing motion and the
 * tap-focus pulse.
 */
export function SanctuaryTransition({
  ...
  const [displayId, setDisplayId] = useState(targetId);
  ...
  return (
    <group ref={outer}>
      <group ref={inner}>
        <SanctuaryModel id={displayId} state={riskState} />
      </group>
