...
import { SANCTUARIES } from "../../data/sanctuaries-c";
 ...
 * `appearance` picks the visual world: the 4.C warm paper sheet, or the
...
export function SanctuarySelector({
  ...
  const gdc = appearance === "product" ? " is-gdc" : "";
    ...
    <AnimatePresence>
      {open && (
        <>
          <motion.button
            type="button"
            className={`sanctuary-c-sheet__backdrop${gdc}`}
          ...
          />
          <motion.div
            className={`sanctuary-c-sheet${gdc}`}
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="sanctuary-c-sheet__grabber" aria-hidden="true" />
            <h2 className="sanctuary-c-sheet__title">
              {showLook ? "Model & look" : "Choose your sanctuary"}
            </h2>
            <p className="sanctuary-c-sheet__hint">
              {showLook
                ? "Pick a form, then tune its color and brightness."
                : "Select the place that represents everything you’re protecting."}
            </p>

            <div className="sanctuary-c-sheet__row">
              {SANCTUARIES.map((s) => {
                ...
                return (
                  <button
                    key={s.id}
                    type="button"
                    className={`sanctuary-c-option${active ? " is-active" : ""}`}
                    aria-pressed={active}
                    onClick={() => onPreview(s.id)}
                  >
                    <span className="sanctuary-c-option__well">
                      <SanctuaryThumbnail id={s.id} />
                      {s.id === committedId && (
                        <span className="sanctuary-c-option__badge">
                          <Check size={10} strokeWidth={3} aria-hidden="true" />
                          Current
                        </span>
                      )}
                    </span>
                    <span className="sanctuary-c-option__name">{s.name}</span>
                    <span className="sanctuary-c-option__descriptor">
                      {s.descriptor}
                    </span>
                  </button>
                );
            ...
            {showLook && (
              <div className="sanctuary-c-sheet__look">
                <p className="sanctuary-c-sheet__look-label">Color</p>
                <div className="sanctuary-c-sheet__colors" role="listbox" aria-label="Accent color">
                  {LOOK_COLORS.map((c) => {
                    ...
                    return (
                      <button
                        ...
                        aria-label={c.label}
                        className={`sanctuary-c-sheet__color${active ? " is-active" : ""}`}
                ...
                <label className="sanctuary-c-sheet__bright">
                  <span className="sanctuary-c-sheet__bright-head">
                    <span>
            ...
            <button
              type="button"
              className="sanctuary-c-sheet__confirm"
              onClick={onConfirm}
            >
