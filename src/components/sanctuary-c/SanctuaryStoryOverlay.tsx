...
export default function SanctuaryStoryOverlay() {
  ...
  return (
    <div className="sc-story" ref={rootRef}>
      {frame && (
        <StoryShell
          frame={frame}
...
function StoryShell({
  ...
  return (
    <motion.div
      className="sc-story__container"
      style={{ left, top, width, height, borderRadius: radius, opacity: veil }}
    >
      <motion.div
        className="sc-story__stage-holder"
        style={{
          width: frame.w,
          height: frame.h,
      ...
      >
        <div
          className={[
            "sc-story__screen",
            theme.ink === "light" ? "sc-story--ink-light" : "",
          ]
            .filter(Boolean)
            .join(" ")}
        >
          ...
          <motion.div
            className="sc-story__chrome"
            style={{ opacity: copyOpacity, pointerEvents: copyEvents }}
          >
            <button
              type="button"
              className="sc-story__chrome-btn"
          ...
          >
            <AnimatePresence mode="wait" initial={false}>
              {activeScene === null ? (
                <motion.div
                  key="overview"
                  className="sc-story__copy"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -8 }}
                  transition={{ duration: 0.26, ease: "easeOut" }}
                >
                  <p className="sc-story__eyebrow">Your Sanctuary</p>
                  <h1 className="sc-story__headline">{story.headline}</h1>
                  <p className="sc-story__body">{story.body}</p>
                  <button
                    type="button"
                    className="sc-story__cta"
                ...
                <motion.div
                  ...
                  transition={{ duration: 0.26, ease: "easeOut" }}
                >
                  <div className="sc-story__progress" aria-hidden="true">
                    {story.scenes.map((scene, i) => (
                      <button
                        key={scene.id}
                        type="button"
                        tabIndex={-1}
                        className={`sc-story__dot${i === sceneIndex ? " is-active" : ""}`}
                        onClick={() => setSceneIndex(i)}
                      />
                    ))}
                    <span className="sc-story__step">
                      {(sceneIndex ?? 0) + 1} of {story.scenes.length}
                    </span>
                  </div>
                  <h1 className="sc-story__headline">{activeScene.headline}</h1>
                  <p className="sc-story__body">{activeScene.body}</p>

                  {sceneIndex !== null && sceneIndex < story.scenes.length - 1 ? (
                    <div className="sc-story__scene-actions">
                      <button
                        type="button"
                        className="sc-story__cta"
          ...
          <div className="sc-story__grain" aria-hidden="true" />
