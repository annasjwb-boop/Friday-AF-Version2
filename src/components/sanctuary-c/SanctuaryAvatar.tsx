...
export function SanctuaryAvatar({ fab = false }: { fab?: boolean }) {
  ...
  const button = (
    <button
      ref={buttonRef}
      type="button"
      className={`sanctuary-c-avatar${fab ? " sanctuary-c-avatar--fab" : ""}${
        storyOpen ? " is-hidden" : ""
      }`}
      aria-label="Open your sanctuary"
      onClick={handleOpen}
    >
      <span className="sanctuary-c-avatar__scene" aria-hidden="true">
  ...
  return (
    <BorderBeam
      ...
      borderRadius={36}
      active={!storyOpen}
      className={`sanctuary-c-fab${storyOpen ? " is-hidden" : ""}`}
