...
const STORAGE_KEY = "aidfinder:sanctuary-c";
...
/**
 * SANCTUARY 4.C — independent fork of the 4.A experience (components,
...
export function SanctuaryHero() {
  ...
  return (
    <section
      className={[
        "sanctuary-c",
        theme.ink === "light" ? "sanctuary-c--ink-light" : "",
      ]
        .filter(Boolean)
        .join(" ")}
    >
      <p className="sanctuary-c__eyebrow">
      ...
      <div
        className={[
          "sanctuary-c__stage",
          view === "risk" ? "sanctuary-c__stage--risk" : "",
        ]
          .filter(Boolean)
          .join(" ")}
      >
      ...
      {view === "choose" ? (
        <div className="sanctuary-c__panel">
          <SanctuaryDetails sanctuary={sanctuary} />

          <ThemePicker />

          <div className="sanctuary-c__actions">
            <button
              type="button"
              className="sanctuary-c__cta"
        ...
        <div className="sanctuary-c__panel sanctuary-c__panel--risk">
          <div className="sanctuary-c-readout">
            <h1 className="sanctuary-c-readout__headline">
              {insight.headline}
            </h1>
            {/* The status line doubles as the hidden demo-control trigger. */}
            <button
              type="button"
              className="sanctuary-c-readout__status"
              onClick={() => setDemoOpen((v) => !v)}
              title="Toggle profile demo"
            >
              {insight.status}
            </button>
            <div className="sanctuary-c-readout__scores">
              {indicators.map((ind) => (
                <div
                  key={ind.id}
                  className={`sanctuary-c-score-chip${
                    ind.unknown ? " is-unknown" : ""
                  }`}
                >
                  <span className="sanctuary-c-score-chip__label">
                    {ind.label}
                  </span>
                  <span className="sanctuary-c-score-chip__value">
                    {ind.value}
                  </span>
                  <span className="sanctuary-c-score-chip__level">
                    {ind.level}
                  </span>
                </div>
              ))}
      ...
      <div className="sanctuary-c__grain" aria-hidden="true" />
