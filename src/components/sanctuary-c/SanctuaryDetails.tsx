...
export function SanctuaryDetails({ sanctuary }: { sanctuary: Sanctuary }) {
  return (
    <div className="sanctuary-c-details">
      <AnimatePresence mode="wait" initial={false}>
        <motion.div
          key={sanctuary.id}
          className="sanctuary-c-details__text"
          initial={{ opacity: 0, y: 14 }}
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0, y: -10 }}
          transition={{ duration: 0.34, ease: [0.3, 0, 0.2, 1] }}
        >
          <h1 className="sanctuary-c-details__title">{sanctuary.name}</h1>
          <p className="sanctuary-c-details__desc">{sanctuary.description}</p>
        </motion.div>
      </AnimatePresence>
      <span className="sanctuary-c-details__status">
        <span className="sanctuary-c-details__status-dot" />
