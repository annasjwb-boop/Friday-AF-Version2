  ...
  return (
    <AnimatePresence>
      {open && (
        ...
        >
          <div className="sanctuary-c-why__head">
            <div>
              <h2 className="sanctuary-c-why__title">
                Why your sanctuary looks this way
              </h2>
              <p className="sanctuary-c-why__hint">
            ...
            <button
              type="button"
              className="sanctuary-c-why__close"
              aria-label="Close"
              onClick={onClose}
            >
          ...
          <div className="sanctuary-c-why__list">
            {annotations.map((a) => {
              ...
              return (
                <button
                  key={a.id}
                  type="button"
                  className={`sanctuary-c-why__row${active ? " is-active" : ""}`}
                  aria-pressed={active}
                  onClick={() => onSelect(active ? null : a.id)}
                >
                  <span className="sanctuary-c-why__row-title">{a.title}</span>
                  <span className="sanctuary-c-why__row-meaning">
                    {a.meaning}
                  </span>
                  {active && (
                    <motion.span
                      className="sanctuary-c-why__row-detail"
                      initial={{ opacity: 0, height: 0 }}
                      animate={{ opacity: 1, height: "auto" }}
                      exit={{ opacity: 0, height: 0 }}
                      transition={{ duration: 0.2, ease: [0.3, 0, 0.2, 1] }}
                    >
                      <span className="sanctuary-c-why__row-fact">{a.fact}</span>
                      <span className="sanctuary-c-why__row-action">
                        {a.action}
                      </span>
                    </motion.span>
                  )}
