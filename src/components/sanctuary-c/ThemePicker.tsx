...
export function ThemePicker() {
  ...
  return (
    <div className="sanctuary-c-themes" role="radiogroup" aria-label="Environment theme">
      {SANCTUARY_THEMES.map((theme) => {
        ...
        return (
          <button
            ...
            role="radio"
            aria-checked={active}
            className={`sanctuary-c-themes__swatch${active ? " is-active" : ""}`}
            onClick={() => setSanctuaryTheme(theme.id)}
          >
            <span
              className="sanctuary-c-themes__dot"
              style={{ background: theme.swatch }}
            />
            <span className="sanctuary-c-themes__label">{theme.label}</span>
          </button>
        );
      })}
    </div>
