...
/**
 * Sanctuary 4.C core screens — a calm system language built from the
...
export function CoreScreens({
  ...
  const goToNextSection = () => {
    ...
    requestAnimationFrame(() => {
      document
        .getElementById(`scc-section-${nextSection.id}`)
  ...
  return (
    <div className="scc">
      ...
      <BorderBeam size="line" colorVariant="ocean" theme="dark">
        <div className="scc-mesh">
          <div className="scc-tabs" role="tablist" aria-label="Home views">
            {TABS.map(({ id, label, icon: Icon }) => (
              <button
                ...
                aria-label={label}
                title={label}
                className={`scc-tabs__tab${id === tab ? " is-active" : ""}`}
                onClick={() => setTab(id)}
              >
      ...
      <div className="scc-sheet">
        <div className="scc-panel" key={tab}>
...
function CircleChevron({ open }: { open?: boolean }) {
  return (
    <span className="scc-circle-chevron" aria-hidden="true">
      {open === undefined ? (
...
function MiniRing({ pct }: { pct: number }) {
  ...
  const c = 2 * Math.PI * r;
  return (
    <svg className="scc-ring" viewBox="0 0 36 36" aria-hidden="true">
      <circle className="scc-ring__track" cx="18" cy="18" r={r} />
      <circle
        className="scc-ring__fill"
        cx="18"
        cy="18"
        r={r}
      ...
      />
      <text className="scc-ring__text" x="18" y="19">
        {pct}%
      </text>
    </svg>
  );
...
function TickBar({
  ...
  return (
    <div className="scc-scale">
      <div className="scc-ticks" aria-hidden="true">
        {Array.from({ length: count }, (_, i) => (
          <span
            key={i}
            className={`scc-ticks__tick${i < filled ? " is-filled" : ""}`}
          />
        ))}
      </div>
      <div className="scc-scale__ends" aria-hidden="true">
        <span>{min}</span>
        <span>{max}</span>
      </div>
    </div>
    ...
    >
      <defs>
        ...
        >
          ...
          <stop offset="0.32" stopColor="#b6b6ba" />
          <stop offset="0.52" stopColor="#ff9636" />
          <stop offset="0.74" stopColor="#ff7500" />
      ...
      <text className="scc-zonebar__num" x="1" y="72" textAnchor="start">
        0
      </text>
      {riskZones.slice(1).map((zone) => (
        <text
          key={zone.id}
          className="scc-zonebar__num"
          x={bx(zone.from)}
          y="72"
          textAnchor="middle"
        >
...
function RiskHero() {
  ...
  return (
    <div className="scc-hero scc-hero--risk">
      <p className="scc-hero__label">Risk score</p>
      <div className="scc-hero__figure-row">
        <p className="scc-hero__figure">{riskScore.value}</p>
        {delta !== 0 && (
          <span
            className={`scc-hero__delta${delta < 0 ? " scc-hero__delta--good" : ""}`}
          >
            {delta > 0 ? "+" : "−"} {Math.abs(delta)} pts
          </span>
        )}
      </div>
      <p className="scc-hero__zone">{riskScore.label}</p>
      <RiskZoneBar value={riskScore.value} max={riskScore.max} />
      <div className="scc-hero__notes">
  ...
  return (
    <>
      <section aria-label="Exposure breakdown">
        <p className="scc-section-label">What's driving it</p>
        <ul className="scc-cards">
          {exposures.map((exposure) => {
            ...
            return (
              <li key={exposure.id}>
                <button
                  type="button"
                  className="scc-card scc-card--button"
                  aria-expanded={expanded}
                  onClick={() =>
                    setExpandedId(expanded ? null : exposure.id)
                  }
                >
                  <span className="scc-card__head">
                    <span className="scc-card__title">{exposure.name}</span>
                    <CircleChevron open={expanded} />
                  </span>
                  <span className="scc-card__body">{exposure.description}</span>
                  {expanded && (
                    <span className="scc-card__detail">{exposure.detail}</span>
                  )}
                  <span className="scc-card__foot">
                    <span className="scc-card__tag">{exposure.meta}</span>
                    <MiniRing pct={share} />
                  </span>
                </button>
              </li>
      ...
      <p className="scc-footnote">
        Scored {riskScore.date}
        <span className="scc-footnote__dot" aria-hidden="true" />
...
function ReadinessHero({
  ...
  return (
    <div className="scc-hero">
      <p className="scc-hero__label">Aid readiness</p>
      <p className="scc-hero__figure">
        {overall}
        <span className="scc-hero__unit">%</span>
      </p>
      <p className="scc-hero__meta">
        ready to apply for aid
        <span className="scc-hero__meta-dot" aria-hidden="true" />
      ...
      <div className="scc-hero__actions">
        {actions.map(({ id, label, icon: Icon, onClick }) => (
          <button
            key={id}
            type="button"
            className="scc-hero__action"
            onClick={onClick}
          >
            <span className="scc-hero__action-icon" aria-hidden="true">
...
function ReadinessPanel({
  ...
  const renderSection = (section: ReadinessSection) => {
    ...
    const done = section.items.filter((item) => item.done).length;
    return (
      <li key={section.id} id={`scc-section-${section.id}`}>
        <button
          type="button"
          className="scc-task"
          aria-expanded={expanded}
        ...
        >
          <span
            className={`scc-task__mark${pct === 100 ? " is-done" : ""}`}
            aria-hidden="true"
          >
            {pct === 100 && <Check size={12} strokeWidth={2.5} />}
          </span>
          <span className="scc-task__labels">
            <span className="scc-task__name">{section.name}</span>
            <span className="scc-task__meta">
              {done} of {section.items.length}
              <span className="scc-task__meta-dot" aria-hidden="true" />
              {pct === 100
                ? "Complete"
                : pct === 0
                  ? "Not started"
        ...
        {expanded && (
          <ul className="scc-task__items">
            {section.items.map((item) => (
              <li
                key={item.id}
                className={`scc-task__item${item.done ? " is-done" : ""}`}
              >
                <span className="scc-task__item-mark" aria-hidden="true">
                  {item.done && <Check size={10} strokeWidth={2.5} />}
                </span>
                {item.name}
                {!item.done && (
                  <span className="scc-task__item-hint">Missing</span>
                )}
              </li>
            ))}
          </ul>
  ...
  return (
    <>
      <div className="scc-stat-pair">
        <div className="scc-stat">
          <p className="scc-stat__label">Documents on file</p>
          <p className="scc-stat__value">
            {documentsDone}{" "}
            <span className="scc-stat__value-sub">of {documentsTotal}</span>
          </p>
        </div>
        <div className="scc-stat">
          <p className="scc-stat__label">Belongings documented</p>
          <p className="scc-stat__value">{formatMoney(documentedValue)}</p>
        </div>
      </div>

      {nextItem && (
        <button type="button" className="scc-next" onClick={onNextStep}>
          <span className="scc-next__labels">
            <span className="scc-next__kicker">Next step</span>
            <span className="scc-next__value">
      ...
      <section aria-label="Readiness by section">
        <p className="scc-section-label">Readiness by section</p>
        <ul className="scc-tasks">
          {readinessSections.slice(0, 3).map(renderSection)}

          {/* The asset library row drills into the library itself. */}
          <li>
            <button type="button" className="scc-task" onClick={onOpenAssets}>
              <span
                className={`scc-task__mark${documentedPct === 100 ? " is-done" : ""}`}
                aria-hidden="true"
              >
                {documentedPct === 100 && <Check size={12} strokeWidth={2.5} />}
              </span>
              <span className="scc-task__labels">
                <span className="scc-task__name">Asset library</span>
                <span className="scc-task__meta">
      ...
      <section aria-label="Asset library">
        <p className="scc-section-label">Asset library</p>
        <button
          type="button"
          className="scc-card scc-card--button"
        ...
        >
          <span className="scc-card__head">
            <span className="scc-card__title">Documented value</span>
            <CircleChevron />
          </span>
          <span className="scc-card__body">
            Document belongings and estimated values once — they pre-fill
            insurance claims and aid applications after a disaster.
          </span>
          <span className="scc-card__foot">
            <span className="scc-card__stat">
              <span className="scc-card__stat-value">
                {formatMoney(documentedValue)}
              </span>
              <span className="scc-card__stat-caption">
...
/** 4.C's blue-and-grey coding: funding sources run ink → grey by weight,
  ...
  return (
    <div className="scc-arc">
      ...
      >
        ...
        <text className="scc-arc__capsule-text" dy="4.5">
      ...
      {segments.map((segment) => {
        ...
        return (
          <span
            key={segment.id}
            className={`scc-arc__icon${LIGHT_SEGMENTS.has(segment.id) ? " scc-arc__icon--ink" : ""}`}
      ...
      <div className="scc-arc__fig">
        <p className="scc-arc__fig-label">
          {uncovered > 0 ? "Uncovered balance" : "Funding secured"}
        </p>
        <p
          className={`scc-arc__fig-amount${uncovered > 0 ? " scc-arc__fig-amount--gap" : ""}`}
        >
...
function RecoveryHero({
  ...
  return (
    <div className="scc-hero" key={view.id}>
      <div className="scc-hero__views" role="tablist" aria-label="Cost view">
        {costViews.map((option) => (
          <button
            ...
            className={`scc-hero__view${option.id === view.id ? " is-active" : ""}`}
      ...
      <p className="scc-hero__figure scc-hero__figure--money">
...
function RecoveryPanel({
  ...
  return (
    <>
      ...
      <div className="scc-outlook__reading" key={view.id}>
        <FundingArc
          view={view}
          uncovered={uncovered}
      ...
      <section aria-label="Funding sources">
        <p className="scc-section-label">Where funding comes from</p>
        <div className="scc-card">
          <ul className="scc-sources">
            {view.sources.map((source) => (
              <li key={source.id} className="scc-source">
                <span
                  className="scc-source__dot"
                  style={{ background: SOURCE_COLORS[source.id] }}
                  aria-hidden="true"
                />
                <span className="scc-source__labels">
                  <span className="scc-source__name">{source.name}</span>
                  <span className="scc-source__detail">{source.detail}</span>
                </span>
                <span className="scc-source__amount">
                  {formatMoney(source.amount)}
                </span>
              </li>
            ))}
            <li className="scc-source scc-source--gap">
              <span
                className="scc-source__dot"
              ...
              />
              <span className="scc-source__labels">
                <span className="scc-source__name">Uncovered balance</span>
                <span className="scc-source__detail">
      ...
      <button type="button" className="scc-cta" onClick={onExploreSupport}>
